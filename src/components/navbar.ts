...
export function renderNavbar() {
  ...
  const user = getUser()
  const logoPath = '/auction-house2.png'

  const defaultAvatar =
    'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?q=80&w=2070&auto=format&fit=crop'

  const userAvatar = user?.avatar?.url ?? defaultAvatar
  const searchInput = `
    <div class="flex items-center bg-gray-200 rounded-lg px-3 py-3 w-full lg:ml-7 lg:max-w-[250px]">
  ...
  return `
    <nav class="flex flex-col fixed top-0 left-0 lg:py-1 w-full z-100 bg-white lg:bg-white/90 backdrop-blur-xl border-b border-gray-200 shadow-sm">
      <div class="container mx-auto border-b-2 border-gray-200 lg:border-0 px-6 py-3 md:pt-3 lg:py-2 flex items-center justify-between">

       <!-- Logo -->
<a href="/" class="flex items-center gap-2 text-2xl sm:text-3xl font-medium tracking-tight">
<img src="${logoPath}" alt="Auction House Logo" class="h-10 w-auto sm:h-12">
</a>
          ...
          <div class="flex items-center justify-center gap-2">
            ...
            ${
              authed
                ...
                <a id="navbarProfile" href="/profile" class="relative flex items-center lg:min-w-fit gap-2 px-4 p-2 mb-2 pb-0 text-indigo-600 font-semibold transition group">
                  ...
                  <span class="truncate max-w-[120px] text-base leading-normal mt-1">Hi, ${
                    user?.name || 'Profile'
                  }</span>
                  </div>
      ...
      <div id="mobileMenu" class="lg:hidden container mx-auto px-6 max-h-0 overflow-hidden transition-all duration-500 ">
        <div class="flex flex-col space-y-3 py-5">
          ...
          <!-- Shared search input for mobile -->
          ${searchInput
